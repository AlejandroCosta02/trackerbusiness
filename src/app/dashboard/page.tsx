...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Only add date filters if both start and end dates are present
      if (filters.startDate && filters.endDate) {
        // Ensure end date includes the entire day by setting it to the end of the day
  ...
  }, [session, business?._id, pagination.page, pagination.limit, filters]);
    ...
    const fetchBusiness = async () => {
      try {
        ...
        console.log('Fetched business data:', {
          name: data[0]?.name,
          hasLogo: !!data[0]?.logo,
          logoPreview: data[0]?.logo?.substring(0, 50) + '...'
        });
