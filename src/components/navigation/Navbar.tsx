...
export function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-md transition-colors duration-300 ${
      isHomePage ? 'bg-transparent' : 'bg-white/70'
    }`}>
        ...
        <div className="flex justify-between h-16">
          <div className="flex">
            <div className="flex-shrink-0 flex items-center">
              <Link 
                href="/" 
                className={`text-xl font-bold ${isHomePage ? 'text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600' : 'text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600'} transition-colors duration-300`}
              >
                Business Tracker
              </Link>
            </div>
            ...
            {session ? (
              ...
              <Link
                ...
                className={`inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 shadow-sm hover:shadow-md ${
                  isHomePage 
                  ? 'text-indigo-600 bg-white/90 hover:bg-white border border-indigo-100' 
